...
(function ($, undefined) {
    ...
    var initializeRowExtEditor = function (t, opts, index) {
        ...
        $("<a></a>").appendTo(p).linkbutton({ plain: false, iconCls: "icon-ok", text: Config.getLang("save") }).click(function () {
            AdminDataGrid.onSaveRowEdit(index, t, opts);

            t.datagrid("endEdit", index);
            disposeRowExtEditor(t, opts, index);
        });
        $("<a></a>").appendTo(p).linkbutton({ plain: false, iconCls: "icon-cancel", text: Config.getLang("cancel") }).click(function () {
            t.datagrid("cancelEdit", index);
            disposeRowExtEditor(t, opts, index);
        });

        var posFocus = tr.find("td[field='"+opts.autoFocusField+"']").position();

//        var diff = (opts.width - p.outerWidth()) / 2 - width, left = diff > 0 ? diff : 0;
        p.css("left",posFocus.left);
    };
