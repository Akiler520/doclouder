    ...
    var ProcessManager = {
        ...
        param: {
            ...
            error: function(XMLHttpRequest, textStatus, errorThrown){
                ...
                for(var key_store in ProcessManager.store){
                    if(!ProcessManager.store.hasOwnProperty(key_store)){
                        continue;
                    }
                    if(XMLHttpRequest == ProcessManager.store[key_store].processXHR){
                        ProcessManager.abort(key_store);
                        break;
                    }
                }
...
//                console.log("readyState:"+XMLHttpRequest.readyState);
         ...
         * @returns {string}
         */
        register: function(keyword, timeout){
            if(typeof keyword == 'undefined'){
                keyword = new Date().getTime();
            }
            ...
            if(typeof  this.store[keyword] == 'undefined'){
                this.store[keyword] = {priority: 0, timeout: timeout, start:0, processXHR:0};
            }
        ...
        run: function(param){
            ...
            if(typeof  this.store[param.key] == 'undefined'){
                param.key = this.register(param.key, param.timeout);
            }
            ...
            ajax_param.beforeSend = function(data, textStatus, XMLHttpRequest){
                if(typeof param.beforeSend != 'undefined'){
                    param.beforeSend(data, textStatus, XMLHttpRequest);
                }else{
                    ProcessManager.param.beforeSend(data, textStatus, XMLHttpRequest);
                }
            };
            ...
            ajax_param.success = function(data, textStatus, XMLHttpRequest){
                ...
                ProcessManager.param.success(data, textStatus, XMLHttpRequest);
            };
            ...
            ajax_param.error = function(XMLHttpRequest, textStatus, errorThrown){
                ...
                ProcessManager.param.error(XMLHttpRequest, textStatus, errorThrown);
            };
            ...
            if(typeof param.loop != 'undefined' && param.loop >= 0){
                ...
                // run it once first
                ProcessManager.store[param.key].processXHR = $.ajax(ajax_param);

                // start to loop
                this.store[param.key].loopBar = setInterval(function(){
                    ProcessManager.store[param.key].processXHR = $.ajax(ajax_param);
                    ProcessManager.store[param.key].loopCount++;
                }, loop_interval);
            }else{
        ...
        manager: function(){
            this.managerBar = setInterval(function(){
                ...
                for(var key_store in ProcessManager.store){
                    ...
                    var processObj = ProcessManager.store[key_store];
        ...
        getProcessByKey: function(key_process){
            for(var key_store in ProcessManager.store){
                ...
                if(key_process == key_store){
                    return ProcessManager.store[key_store];
                }
            }
