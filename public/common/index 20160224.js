(function ($) {
    ...
    var homePageTitle = Config.getLang("home_page"), homePageHref = null, navMenuList = "#navMenu_list",
    ...
    window.mainpage.loadNavMenus = function (callback) {
        ...
        //$.get("/Public/common/nav-menu-data.json", function (menus) {
        $.get("/Menu/main/", function (menus) {
            $.each(window.mainpage.navMenusData = menus = $.array.filter(menus, function (item) {
                ...
                $("<span class=\"l-btn-left l-btn-icon-left nav-menu\">" +
                    "<span class=\"l-btn-text\">" + (item.text ? item.text : "无标题") + "</span>" +
...
//                console.log(node);
    ...
    window.mainpage.loadFavoMenus = function () {
        ...
        $(favoMenuTree).tree("load", "/Public/common/favo-menu-data.json");
    };
    ...
    window.mainpage.bindToolbarButtonEvent = function () {
        ...
        $(uploadBtn).click(function () {
            var treeNodeSelected = $(navMenuTree).tree("getSelected");
            ...
            var folderId = 0;   // current selected folder;

            folderId = treeNodeSelected.id;
            ...
            $.easyui.messager.confirm("操作提醒", "您确定要退出当前程序并关闭该网页？", function (c) {
                if (c) {
                    ...
                    $.ProcessManager.run({
                        url: "/User/end",
                        key: "User-end",
                        beforeSend: function (XMLHttpRequest) { $.easyui.loading({ locale: orderUnprocessed }); },
                        ...
                        complete: function (XMLHttpRequest, textStatus) { $.easyui.loaded(orderUnprocessed); }
                    });
                }
            });
        });
...
//        if (!opts.href) { return; }
    ...
    window.mainpage.mainTabs.loadHash = function (hash) {
        ...
        $.get("/Public/common/nav-api-menu-data.json", function (menus) {
    ...
    // 初始化待处理订单，已经支付的订单
    $.util.namespace("orderUnprocessed");
    window.orderUnprocessed.reload = function(){
        var orderUnprocessed = $("#order-list-unprocessed");

        $.ProcessManager.run({
            url: "/Order-unprocessed",
            key: "Order-unprocessed",
            beforeSend: function (XMLHttpRequest) { $.easyui.loading({ locale: orderUnprocessed }); },
            success: function (data, textStatus, XMLHttpRequest) {
                $(orderUnprocessed).html("");
                for(var key in data){
                    if (data.hasOwnProperty(key)) {
                        $("<p style='padding: 4px;'>["+data[key].order_sn+"][￥"+data[key].order_amount+"] 支付时间："+Common.format(data[key].pay_time)+"</p>").appendTo(orderUnprocessed);
                    }
                }
            },
            complete: function (XMLHttpRequest, textStatus) { $.easyui.loaded(orderUnprocessed); }
    ...
    // 初始化待处理订单，新订单，未支付
    $.util.namespace("orderUnpaidNew");
    window.orderUnpaidNew.reload = function(){
        var orderUnprocessedNew = $("#order-list-new");

        $.ProcessManager.run({
            url: "/Order-upPaid",
            key: "Order-upPaid",
            beforeSend: function (XMLHttpRequest) { $.easyui.loading({ locale: orderUnprocessedNew }); },
            success: function (data, textStatus, XMLHttpRequest) {
                $(orderUnprocessedNew).html("");
                for(var key in data){
                    if (data.hasOwnProperty(key)) {
                        $("<p style='padding: 4px;'>["+data[key].order_sn+"][￥"+data[key].order_amount+"] 下单时间："+Common.format(data[key].create_time)+"</p>").appendTo(orderUnprocessedNew);
                    }
                }
            },
            complete: function (XMLHttpRequest, textStatus) { $.easyui.loaded(orderUnprocessedNew); }
        });
    };
